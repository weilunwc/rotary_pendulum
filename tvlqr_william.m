...
x_nom =xr';
u_nom = u_sol;
A_traj = zeros(16,size(time,2));
B_traj = zeros(4,size(time,2));

for i = 1:length(time)
    [A,B] = linearizedfurata_z(x_nom(i,:),u_nom(i));
    A_traj(:,i) = A(:);
    B_traj(:,i) = B(:);
end
...
Qf = 100*eye(4);
[emit,Sflipped] = ode45(@(t,y)Sdynamics(t,y,Q,R,A_traj,B_traj,x_nom,u_nom,time),[time(end) 0],Qf(:));
